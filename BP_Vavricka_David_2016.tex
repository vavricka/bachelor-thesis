...
Za použití tohoto dému není možné splnit některé stěžejní technické požadavky, jako například "Snížení objemu logů" nebo "Post processing zpráv". Proto bude nutné ho nahradit nějakým vyspělejším démonem. Tím se ale zabývá až další kapitola "Volba postupu řešení".

\subsection{Browser}
Browser je rozsáhlá v javascriptu napsaná aplikace, která - TODO co dělá? -

 kromě jiných funkcionalit slouží k vzdálenému ovládání STB. Naslouchá zprávám z aplikačního serveru, v závislosti na kterých vygeneruje POST či GET request pro démon dmd.
Browser generuje značné množství logů jejichž nevýhodou je, že nepoužívají standardizovanou syslog množinu severit, ale svou vlastní s ní nekompatibilní. Implementaci browseru v této práci bohužel není možné měnit a proto je třeba nekompatibilitu vyřešit pomocí logovacího démonu démonu.

\subsection{Servisní komponenta dmd}
Dmd je v C++ napsaná servisní komponenta, která zprostředkovává komunikaci mezi Browserem a shellovým prostředím a umožňuje tak servisním technikům přistupovat na dálku k shellu STB. Výsadou dmd je jeho snadno rozšiřitelné API.

\subsection{Servery zadavatele}
TODO-Matěj nějak popsat ty servery, když už jsou na UML diagramu tak je třeba je popsat END-TODO
Aplikační servery starající se o bussines logiku není možné v této práci jakkoli měnit. Je nutné zachovat kompatibilitu.

\section{Buildovací systém Gu}
...
\section{PKGBUILD}
Jedn8 se shellový script obsahující informace potřebné pro build jednotlivých aplikací systémem Gu. Níže je pro představu uveden vzorový script:
\begin{scriptsize}
\begin{verbatim}
pkgname=libee
pkgver=0.4.1
...
Tento démon je již popsán v předešlé kapitole.
